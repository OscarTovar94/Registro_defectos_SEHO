...
def defect_root():
    ...
    def dashboard_seho():
        ...
        try:
            ...
            fig1 = plt.Figure(figsize=(4, 4), facecolor="#0F111A")
            ...
            ax1.text(0, -0.45, f"STATUS: {estado}", ha='center', va='center', fontsize=5, color=color_kpi,
                     fontweight='bold')
            ax1.set_title(f"Calidad vs Meta ({fpy_meta}%)", pad=15)

            FigureCanvasTkAgg(fig1, main_container).get_tk_widget().grid(row=0, column=0, sticky="nsew", padx=5)

            # --- GRÁFICA 2: PRODUCCIÓN TOTAL (STACKED BAR) ---
            fig2 = plt.Figure(figsize=(5, 3), facecolor="#0F111A")
            ax2 = fig2.add_subplot(111)
            ax2.bar(["Producido"], [buenos], color="#2ECC71", label="OK")
            ax2.bar(["Producido"], [total_defectos], bottom=[buenos], color="#E74C3C", label="Defectos")
            ax2.set_title(f"Unidades Totales: {total_producido}", fontsize=12)
            ax2.legend()
            for i, v in enumerate([buenos, total_defectos]):
                ax2.text(0, v / 2 if i == 0 else buenos + v / 2, f"{int(v)}", ha='center', color='white',
                         fontweight='bold')
            ...
            bar_buenos = ax5.bar(modelos, buenos_val, color="#2ECC71", label="Buenos")
            bar_malos = ax5.bar(modelos, defectos_val, bottom=buenos_val, color="#E74C3C", label="Malos")

            ax5.set_title("Producción Detallada por Modelo", fontsize=12, color="white", pad=20)
            ax5.legend(loc="upper right", fontsize=9)

            # USAMOS .iloc PARA EVITAR EL FUTUREWARNING
            for i in range(len(modelos)):
                total = totales.iloc[i]  # <--- Cambio: usar .iloc
                b = buenos_val.iloc[i]  # <--- Cambio: usar .iloc
                d = defectos_val.iloc[i]  # <--- Cambio: usar .iloc
                ...
                if total > 0:
                    ...
                    ax5.text(i, total + (total * 0.02), f"Total: {int(total)}",
                             ha='center', va='bottom', color='#00D4FF', fontsize=10, fontweight='bold')
